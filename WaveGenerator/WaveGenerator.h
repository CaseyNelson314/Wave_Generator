...
class WaveGenerator {
  ...
  public:
    ...
    double GetSine(double xPhase = 0, double yPhase = 0) {//(ms)(ms)
      return amplitude * (sin((time - lastTime - xPhase) * 2 / cycle * PI) + 1) / 2 + yPhase;
    }

    double GetSquare(double xPhase = 0, double yPhase = 0, byte dutyRatio_Per = 50) {
      return !(fmod(time - lastTime + (cycle - xPhase), cycle) > cycle * dutyRatio_Per / 100) * amplitude + yPhase;
    }
